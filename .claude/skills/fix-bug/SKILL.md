...
You are the orchestrator for the `fix-bug` skill. Take a JIRA bug ticket
...
`/fix-bug https://zegons.atlassian.net/browse/AIDEV-123` or
`/fix-bug AIDEV-123`.
...
- **`LOCAL-EXISTS`** — stop. Report as text (do not route through `echo`
  ...
  > fix-bug from it. No automatic reset or reuse.
...
- **`REMOTE-EXISTS`** (with `LOCAL-ABSENT`) — stop. Report as text:
  ...
  > fix-bug from it, or delete the remote branch before re-running. No
  > automatic reset or reuse.

- **Both `ABSENT`** — create it from the remote default branch. Resolve the
  default branch the same way the `review` skill does (no `gh` required):
...
> This fix looks larger than `fix-bug` is meant for: {one-sentence reason,
> naming the criterion that fired}.
>
> The usual move here is to stop and run `write-design-doc` (or
> `write-design-doc-max`) — a fix this size benefits from a design others
...
- **Anything else — escalate.** Outcome: "escalated". After committing,
  ...
  > `fix-bug` is not the right tool — investigate further and run
  > `write-design-doc` (or `write-design-doc-max` for the deep-review
...
placeholder text remaining. The record is
lightweight institutional memory, nothing more. Interface contracts,
alternatives, and trade-off analysis belong to `write-design-doc`, not
...
- Frontmatter: `ticket: {TICKET}`, `branch:` = the branch from Stage 2.
...
- **The size gate is the engineer's call, not yours.** Too big → stop and
  ...
  `write-design-doc`/`write-design-doc-max`, or wave the gate through when
