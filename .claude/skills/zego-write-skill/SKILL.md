...
You are the orchestrator for `zego-write-skill`. You guide the production of a
...
Check the argument passed to `zego-write-skill`:

- **If a name is given** (e.g. `zego-write-skill audit-repo`): use it as the
...
**Every library skill carries a `zego-` prefix** (per
[ADR 017](../../docs/decisions/017-zego-skill-prefix.md)): the directory name,
the `SKILL.md` `name:` field, and the `/<name>` invocation trigger are all equal
to `zego-<name>`. If the confirmed name does not already start with `zego-`,
prepend it — so an `audit-repo` input becomes the skill `zego-audit-repo`. Use
this prefixed `{name}` everywhere below (directory, `name:` field, paths). On
review, reject any skill directory or `name:` field that is not `zego-`-prefixed
or whose `name:` field does not equal its directory name.
...
Read `.claude/skills/zego-write-skill/persuasion-principles.md`. Classify the skill as one
of:
...
Read `.claude/skills/zego-write-skill/anthropic-best-practices.md` for structural guidance:
...
Read `.claude/skills/zego-write-skill/testing-skills-with-subagents.md` for the full TDD
methodology.
...
Read `.claude/skills/zego-write-skill/persuasion-principles.md` to inform the persuasion
...
Read `.claude/skills/zego-write-skill/testing-skills-with-subagents.md` for the REFACTOR
methodology.
...
- **If CONSUMER** (no `standards/base/` source directory): skip this stage. In a
  consumer repo `/zego-write-skill` adds the team's own extension skills, which are
