...
DETECTORS = [
    ...
    (
        "financial-identifier",
        "high",
        rx(r"\b(iban|sort[_ ]?code|account[_ ]?number|beneficiary|payee)\b\s*[:=]\s*['\"]"),
        "added",
    ),
    ...
    (
        "download-exec",
        "high",
        rx(r"\b(curl|wget|Invoke-WebRequest|iwr)\b[^\n]*(-o|-O|>)[^\n]*\.(sh|ps1|py|exe)\b"),
        "added",
    ),
    ...
    (
        "iac-exposure",
        "high",
        rx(r"(hostNetwork|hostPID|hostIPC|privileged|allowPrivilegeEscalation)\s*:\s*true"),
        "added",
    ),
    ...
    (
        "money-math",
        "medium",
        rx(r"\b(int|float|Number)\s*\([^)]*(amount|price|fee|interest|premium|balance|payout)"),
        "added",
    ),
    (
        "money-math",
    ...
    (
        "logic-bomb-env",
        "medium",
        rx(r"\b(CI|GITHUB_ACTIONS|BUILDKITE|PYTEST_CURRENT_TEST|JENKINS)\b[^\n]*(==|!=|in\b)"),
        "added",
    ),
    ...
    (
        "obfuscation",
        "medium",
        rx(r"(String\.fromCharCode|fromCharCode|\\x[0-9a-fA-F]{2}(\\x[0-9a-fA-F]{2}){6,})"),
        "added",
    ),
...
SELF_SKILL_PATH = re.compile(r"^(\.claude/)?skills/audit-financial-integrity/")
...
class Lead:
    ...
    pattern: str
    snippet: str
    self_referential: bool = False  # lead is in this skill's own files (catalogue/detector example)
...
def _secret_is_noise(content: str) -> bool:
    ...
    if shannon_entropy(val.group(1)) < 3.0 and not SECRET_NAME.search(content):
        return True
    return bool(
        re.search(r"(example|changeme|placeholder|dummy|xxxx|your[_-]?|test_)", val.group(1), re.I)
    )


def _is_noise(category: str, pattern: re.Pattern[str], m: re.Match[str], content: str) -> bool:
...
def _detector_leads(path: str, lineno: int, change: str, content: str, stripped: str) -> list[Lead]:
    ...
    for category, sev, pattern, scope in DETECTORS:
        ...
        if not m or _is_noise(category, pattern, m, content):
            continue
        leads.append(Lead(path, lineno, change, category, sev, m.group(0)[:80], stripped[:200]))
    return leads


def _luhn_leads(path: str, lineno: int, change: str, content: str, stripped: str) -> list[Lead]:
...
def _run_osv_scanner(changed_files: list[str]) -> list[dict[str, Any]]:
    ...
    manifests = [f for f in changed_files if f and f.rsplit("/", 1)[-1] in MANIFEST_NAMES]
...
def run_external_tools(diff_text: str, changed_files: list[str]) -> list[dict[str, Any]]:
...
def main() -> int:
    ...
    ap.add_argument("--diff", required=True, help="Path to a unified diff, or '-' for stdin.")
    ap.add_argument("--repo", default=".", help="Repo root (for optional external tools).")
    ap.add_argument("--out", default="-", help="Output JSON path, or '-' for stdout.")
    ap.add_argument("--no-external", action="store_true", help="Skip external scanners.")
    args = ap.parse_args()
    ...
    leads.sort(key=lambda lead: (sev_order.get(lead.severity_hint, 9), lead.file, lead.line))
