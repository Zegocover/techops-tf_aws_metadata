...
You are the orchestrator for the `zego-review` skill. Review the current branch diff against Zego coding standards as defined in `docs/ai/steering/base/code-review.md`. You do not fix anything — review, record, and return a verdict.
...
3. **Reject any other `--`-prefixed token.** Scan the remaining tokens. If
   ...
   > Unrecognised flag: `{token}`. The only flag `zego-review` accepts is `--no-handoff-gate`. Did you mean `--no-handoff-gate`?
...
There is no empty-residual check for `zego-review`: `zego-review` takes no positional
...
This stage runs ONLY when `zego-review` is invoked standalone. It is a **pure
...
`.claude/skills/zego-fix-pr-comments/SKILL.md`, `.claude/skills/zego-fix-buildkite/SKILL.md`,
`.claude/skills/zego-fix-bug/SKILL.md` Stage 7a) invoke
...
- `NONE`, `CLOSED`, `DRAFT`, or `GH_FAIL` → HALT. Print the canonical
  ...
  `{skill_name}` = `zego-review`, `{branch}` = the current branch, and (for
...
- **K** — active only if the changed file list contains any `.swift` files
- **L** — always active. The Group L sub-agent invokes the `zego-audit-financial-integrity` skill against the diff (see Stage 4 brief). If the `zego-audit-financial-integrity` skill is missing from `.claude/skills/`, record a validation error and skip Group L.
- **M** — active only if the changed file list contains any `.kt` files
- **N** — active only if the changed file list contains any `.proto` files. Also confirm `docs/ai/steering/domains/protobuf-authoring.md` exists — if the trigger fires but the file is missing, record a validation error and skip Group N.
...
> Note on interaction with `docs/ai/steering/base/pull-requests.md` rule 9: a `PRE-IMPLEMENTATION` verdict does not endorse running `zego-review` on WIP branches — rule 9 still applies.
...
- **Group K:** Standard file is `docs/ai/steering/languages/swift.md`. Check every new or modified Swift file against the Swift conventions (force-unwrap/cast prohibition, async/await, @MainActor, weak self, XCTest AAA, mock-at-the-DI-boundary, snapshot tests, format-before-lint).
- **Group L:** No standard rule file in the per-group sense — Group L delegates to the `zego-audit-financial-integrity` skill. Instruct the sub-agent to: (1) invoke the `zego-audit-financial-integrity` skill (its SKILL.md is at `.claude/skills/zego-audit-financial-integrity/SKILL.md`) against the current diff, providing the same diff inputs the other groups receive; (2) parse the skill's verdict and findings — the skill produces a structured set of findings with severities (critical / high / medium / low); (3) translate into the standard `## Output format` from `code-review.md` using the mapping defined in Group L's section: critical/high → `[blocker]`, medium → `[error]`, low → `[warning]`; (4) emit a single `[pass]` line if the audit returns a clean verdict; (5) preserve the skill's never-approve discipline — Group L's findings exist to make suspicious patterns visible to the human reviewer, not to autonomously block or approve. Do not flatten the skill's threat-catalogue context — quote the relevant catalogue and issue text in each YAML finding's `issue` field. If the `zego-audit-financial-integrity` skill is not present in `.claude/skills/`, record a validation error and emit one `[advisory]` line stating the skill is missing; do not fail the verdict on tool absence.
- **Group M:** Standard file is `docs/ai/steering/languages/kotlin.md`. Check every new or modified Kotlin file against the Kotlin conventions (cancellation-safe coroutine error handling, injected I/O dispatcher, cold Flows, sealed interface over sealed class, data object variants, extension-function mappers, nullable-return-type contract, runTest + advanceUntilIdle, injected test dispatcher).
- **Group N:** Standard file is `docs/ai/steering/domains/protobuf-authoring.md`. Check every new or modified `.proto` file against the contract-authoring rules (no cross-service message imports, reuse of shared `zego.protobuf` common types, contract-level comments, don't duplicate the source of truth, field optionality and numbering, the success/failure result model, verify the real shape). Honour the file's Applicability section: the validation-workflow rules (`buf breaking` against `origin/main`, no whole-file `buf format -w`, validate-before-commit) apply only when the diff is in a protobuf repository running the `buf` toolchain.
...
**Resolve the AI-native banner.** The findings file is AI-native
(`docs/ai/steering/base/review-audience.md`). It has no YAML frontmatter, so the
banner is the **first line of the file**, before the `# Code review …` H1.
Resolve `{surface}` and `{link}` from the design-doc detection already done in
Stage 2:

- If `DESIGN_DOC_PATHS` is non-empty, `{surface}` = `design document` and
  `{link}` = the first path in `DESIGN_DOC_PATHS` (a `docs/design/{TICKET}-*.md`
  file).
- Otherwise — no design document exists for the ticket — `{surface}` =
  `JIRA ticket` and `{link}` = `https://zegons.atlassian.net/browse/{TICKET}`
  (always resolvable).

Emit the resolved blockquote as the first line, with no `{` placeholder token
remaining (example shown with a design-doc link):
...
**Self-check the banner before finalising.** After writing the file, confirm the
resolved AI-native banner is its first line with no placeholder remaining:

```bash
rg -q "^> \*\*AI-native artefact\.\*\* " docs/ai/reviews/{TICKET}-{slug}-{revision}.md && ! rg -q "^> \*\*AI-native artefact\.\*\*.*\{" docs/ai/reviews/{TICKET}-{slug}-{revision}.md && echo BANNER_OK || echo BANNER_MISSING
```

Anything other than `BANNER_OK` — banner absent or a literal `{surface}` /
`{link}` placeholder still present — must be fixed (re-write the file with the
resolved banner as the first line) before reporting in Stage 7. The banner is a
leading blockquote: it does not match the `### F{n} —` finding lines, so it does
not disturb `zego-fix-bug` Stage 7c's finding extraction.
...
- **Group N is silently skipped when no `.proto` files are in the diff.** Record a validation error only if the trigger fires but the standard file is missing.
