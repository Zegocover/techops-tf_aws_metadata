...
Entered when the `Detect input type` section routes to orchestration mode. Runs all task specs for the ticket in dependency order using the single-task implement flow (Stages 0–8) per task via sub-agent. Does not reimplement any stage.
...
> No design doc found for ticket `{TICKET}` (searched `docs/design/` for both `^JIRA:` and `^Ticket:` headers). `zego-implement` requires a design doc to verify the design-phase PR handoff. If this ticket legitimately has no design doc (e.g. a spike), re-run with `--no-handoff-gate`.
...
Fill `{phase_name}` = `design`, `{skill_name}` = `zego-implement`, `{branch}` =
...
**Recover the feature identifier (AIDEV-188 / ADR 020).** Once the gate passes,
recover the shared identifier from the resolved design doc — best-effort, never
mints, never blocks:
...
Compute `predecessor-pr-exists` from the design branch the gate just resolved —
no second lookup. A passing sentinel (`OPEN`, `MERGED`) is `true`; an `OVERRIDE`
or a `GH_FAIL` is the LOST-safe default `true` (a `gh` failure is NEVER read as
"no predecessor PR" — that would let a truly-lost id fall through to MINT and
duplicate the identifier, breaking FR-02). On an empty recover, run
`.claude/scripts/feature-id.sh decide "" true` (always `LOST` here) and proceed
without an id. The recovered `FEATURE_ID` is carried into each per-task spec by
that task's own sub-agent at its `task-implementer.md` Stage 0f (recover from
the design doc → write `feature-id:` into the task-spec frontmatter), so each
implementation PR stamps the same identifier. OM-3 does not mutate any task spec
itself; the per-task Stage 0f is the single carry-in point.
...
```
You are running the `zego-implement` skill for a single task.
Read `.claude/skills/zego-implement/SKILL.md` and execute it starting from Stage 0.
...
Execute Stages 0 through 8 in full. Return when Stage 8 completes or the task
...
7. Summary artefact path (the implementation-summary artefact produced by
   Stage 5; empty string if Stage 5 failed to produce one).
```
...
strict semantics defined in `.claude/skills/zego-implement/SKILL.md`: it strips
...
>
> CI validation hit an authentication precondition — an environment problem, not a code failure. Authenticate, then re-invoke `zego-implement {TICKET}` — completed and skipped tasks will not repeat. No sub-agent is re-spawned here.
...
> re-run `zego-implement {TICKET}` — completed and skipped tasks will not repeat.

Do not proceed to subsequent tasks.

Record each result (filename, status: `complete` / `skipped` / `failed`, PR URL, summary artefact path, iteration count) for OM-6.

### OM-6 — Final summary

Print a summary table of all tasks in execution order:

| Task | Status | PR | Summary | Iterations |
|------|--------|----|---------|------------|
| {filename} | complete | {URL} | {summary artefact path} | {N} |
| {filename} | skipped | — | — | — |
| {filename} | failed | — | — | {N} |

The Summary column renders the summary artefact path captured from each
completed task's sub-agent return item 7. An absent or empty item 7 (Stage 5
failed to produce an artefact, or the task did not reach Stage 5) renders as
`—`.
...
- **Orchestration mode does not run Stages 0–8 itself.** It discovers tasks, validates the graph, and spawns one sub-agent per task; each sub-agent runs the full single-task implement flow from Stage 0.
