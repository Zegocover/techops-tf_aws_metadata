...
Shared principles, severity vocabulary, emission gate, and finding schema for every `zego-write-design-doc-max` check agent. The `review-gate.js` workflow script's check agents each Read the full contents of this document themselves, ahead of their own rubric file and the assembled context. Apply these principles whether the artefact under review is a design document or a task spec.
...
2. Calibrate scepticism to the current review round (the per-round check-agent prompt declares which round you are on):
   - **Round 1** — review exhaustively: walk every rubric item against the artefact before returning. An empty findings array is valid only after that full pass — but do not pad the return with marginal findings that fail the emission gate; every emitted finding costs an engineer round-trip.
...
- `Spec` (required, nullable): when multiple task specs are under review, the filename of the affected spec; null for design-gate findings and genuinely cross-spec findings. Always set it explicitly — use null rather than omitting the field.

Or empty list if no findings.

## Field budgets

The three prose fields are disjoint — each carries information the other two do not, with no restatement between them. Every word of a finding is output tokens you generate and prose in the durable findings file: overlapping or padded fields slow the gate and bury the signal.

- `Issue`: the gap, at most 2 sentences. No background, no quoting back the artefact's text.
- `Why it matters`: the consequence only, at most 2 sentences. Never restate the Issue.
- `Suggested resolution`: the change itself — what to add, edit, or remove, and where. Not the rationale again.
