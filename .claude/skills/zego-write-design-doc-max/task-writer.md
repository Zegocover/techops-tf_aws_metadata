...
- `FEATURE_ID` (optional): the shared feature identifier (AIDEV-188 / ADR 020). When provided and non-empty, emit it as the frontmatter `feature-id:` key; when absent or empty, omit the key entirely
...
- AI-native banner: the task spec is AI-native (`docs/ai/steering/base/review-audience.md`); the template carries the banner blockquote as its first content (after the frontmatter `---`, before the `# ` H1). Resolve its placeholders — `{surface}` = `design document`, `{link}` = `DESIGN_PATH` — so the emitted blockquote reads `> **AI-native artefact.** Human reviewers do not need to read this; the review surface for this phase is the design document at {DESIGN_PATH}.` with `DESIGN_PATH` interpolated and no `{` placeholder token remaining
- Frontmatter: `ticket:` and `branch:`, plus an OPTIONAL `feature-id:` third key (the AIDEV-188 / ADR 020 identifier) — no other fields, no `description:` field. Emit `feature-id: {FEATURE_ID}` only when a non-empty `FEATURE_ID` is provided; omit the key entirely when `FEATURE_ID` is absent or empty (do not write an empty value). `branch:` value is exactly `BRANCH` as provided — do not re-derive it from `OUTPUT_PATH`, and do not re-derive it from the design document's `Branch:` line
...
5. Set `branch:` value to `BRANCH` as provided; when a non-empty `FEATURE_ID` is provided, also emit `feature-id: {FEATURE_ID}` in the frontmatter (omit the key when `FEATURE_ID` is absent or empty)
